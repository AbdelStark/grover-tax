# Results: v0.1.0 — 2026-05-20T13:38:49Z
...
|---|---|---|---|
| Proof gen median (n=10 / 10) | 753.065 s | 298.755 s | 2.52× |
| Proof gen IQR | 0.388 | 0.265 | n/a |
| Proof gen min / max | 752.736 / 754.559 | 298.429 / 299.338 | n/a |
| Verifier median (n=50 / 50) | 190349.899 ms | 58.207 ms | 3270.21× |
...
4. **Statement under proof (v0.1).** Both sides prove the *same* statement against the v0.1 fixture — `gate_count` modular additions over secp256k1's prime field:

   ```
   p        = 2^256 − 2^32 − 977            (secp256k1 prime)
   σ_0      = H(circuit_bytes) reduced mod p   (H is each prover's RFC-0005 native hash)
   σ_{i+1}  = (σ_i + (i + 1)) mod p          for i ∈ [0, gate_count)
   publish  = (H(circuit_bytes), gate_count, σ_N)
   ```

   **SP1**: `third_party/sp1/program/src/main.rs` runs the loop natively on the RISC-V zkVM; commits SHA-256 of the circuit bytes plus the final σ_N. `third_party/sp1/verifier/verifier.rs` re-derives σ_N from the fixture and asserts the proof's committed value matches.

   **Stwo (Cairo 1)**: `stwo-side/cairo/src/lib.cairo::apples_to_apples_executable` runs the same loop in Cairo 1 (`u256` modular arithmetic). The compiled `executable.json` is wrapped as a `Task::Cairo1Program` task inside `proving-utils`'s `simple_bootloader_compiled.json`, which `stwo-cairo` proves end-to-end as a Circle STARK. The Cairo function asserts the computed σ_N matches the verifier-provided expected σ_N (passed via the bootloader's user_args). `bin/apples-prove` orchestrates the pipeline; `bin/run_stwo.sh` calls into it.

   The two sides differ only in the per-side native commitment hash (the documented RFC-0005 axis above). The arithmetic loop, the field, the iteration count, and the published output set are identical — **real apples-to-apples** at v0.1's single-laptop budget.

5. **Thread fan-out.** Both provers were invoked with `RAYON_NUM_THREADS=1`, `TOKIO_WORKER_THREADS=1`, `OMP_NUM_THREADS=1`, plus OS-level affinity (taskpolicy -c utility). Observed user-CPU / wall-clock ratios: 0.97 (SP1), 0.93 (Stwo).
...
- Generator: `analyze.py` from commit `74999311e5965e6c9346110065b0224eee3a6c20`
